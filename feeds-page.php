...
class FeedWordPressFeedsPage extends FeedWordPressAdminPage {
	...
	/*static*/ function global_feeds_box ($page, $box = NULL) {
		...
		<option value="shutdown"<?php echo (!$automatic_updates=='shutdown')?' selected="selected"':''; ?> selected >automatically check for updates after pages load</option>
		...
		<tr id="pause-resume" style="display:none;">
...
<?php endforeach; ?>
	...
	function feed_information_box ($page, $box = NULL) {
		...
		<?php
			...
			endforeach;
		?></select></p>
		<p class="setting-description">Editmeet will syndicate the
		...
		<td><label><input type="checkbox" name="hardcode_name" value="no"<?php echo (($hardcode['name']=='yes')?'':' checked="checked"');?> onclick="return false" checked /> Update contributor titles automatically when the feed title changes</label></td>
		</tr>
		
		<tr>
		<th scope="row">Short descriptions:</th>
		<td><label><input type="checkbox" name="hardcode_description" value="no"<?php echo (($hardcode['description']=='yes')?'':' checked="checked"');?> onclick="return false" checked /> Update contributor descriptions automatically when the feed tagline changes</label></td>
		</tr>
		
		<tr>
		<th scope="row">Homepages:</th>
		<td><label><input type="checkbox" name="hardcode_url" value="no"<?php echo (($hardcode['url']=='yes')?'':' checked="checked"');?> onclick="return false" checked /> Update contributor homepages automatically when the feed link changes</label></td>
		</tr>
		...
		if (count($feeds) > 0):
			...
			foreach ($feeds as $key => $f):
				...
				?>
					...
					<div class="submit"><input type="submit" class="button-primary" name="Use" value="&laquo; Subscribe to Editmeet Press Releases" />
	...
	function display_alt_feed_box ($lookup, $params = false) {
		...
		<div class="inside" style="display : none;"><?php
