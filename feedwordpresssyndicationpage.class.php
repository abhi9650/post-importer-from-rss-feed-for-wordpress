...
class FeedWordPressSyndicationPage extends FeedWordPressAdminPage {
	...
	function dashboard_box ($page, $box = NULL) {
		...
		// Hey ho, let's go...
		?>
		<div style="float: left; background: #F5F5F5; padding-top: 5px; padding-right: 5px;"><a href="<?php print $this->form_action(); ?>"><img src="<?php print esc_url(plugins_url( "feedwordpress-tiny.png", __FILE__ ) ); ?>" alt="" /></a></div>

		<p class="info" style="margin-bottom: 0px; border-bottom: 1px dotted black;">Managed by <a href="https://editmeet.com">Editmeet</a>
		<?php print FEEDWORDPRESS_VERSION; ?>.</p>
		<?php if (FEEDWORDPRESS_BLEG) : ?>
		<p class="info" style="margin-top: 0px; font-style: italic; font-size: 75%; color: #666;">You are going to subscribe to Editmeet Press Releases.Click to visit 
		<a href="https://editmeet.com/">Editmeet</a>.</p>
		<br style="clear: left;" />
		<?php endif; ?>
		...
		<div id="add-single-uri">
		  ...
		  <label for="add-uri">Click "+" to subscribe to Editmeet Press Releases:
		  <input type="text" name="lookup" id="add-uri"
		  value="https://editmeet.com/feed/" style="width: 55%;" readonly /></label>
	...
	function syndicated_sources_box ($page, $box = NULL) {
		...
		<div id="add-single-uri" class="alignright">
		  ...
		  <div><?php FeedWordPressCompatibility::stamp_nonce('feedwordpress_feeds'); ?></div>
		  <ul class="subsubsub">
		  <li><label for="add-uri">Editmeet:</label>
		  <input type="text" name="lookup" id="add-uri" value="https://editmeet.com/feed" readonly />
