...
<?php foreach ($matchUl['cats'] as $name => $li) : ?>
	<li><label><input type="checkbox" onclick="return false"
...
<?php	endif; ?>
	...
	<?php foreach ($unmatchedColumns['category'] as $index => $column) : ?>
		...
		<?php foreach ($column as $name => $li) : ?>
			<li><label><input type="radio" onclick="return false" name="unfamiliar_category" value="<?php print $name; ?>"<?php print $unmatchedRadio['category'][$name]; ?> /> <?php print $li['label']; ?></label></li>
		<?php endforeach; ?>
		</ul></td>
	<?php endforeach; ?>
	</tr>
